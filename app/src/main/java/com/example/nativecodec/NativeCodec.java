...
public class NativeCodec extends Activity {
    ...
    public void onCreate(Bundle icicle) {
        ...
        mSurfaceView1 = (SurfaceView) findViewById(R.id.surface);
        mSurfaceView1.getHolder().addCallback(new SurfaceHolder.Callback() {
            @Override
            public void surfaceCreated(SurfaceHolder surfaceHolder) {
                setSurface(surfaceHolder.getSurface());
                try {
                    String nexus7Path = "/sdcard/DCIM/Video_1280x720_F57.mp4";
                    String path5880 = "/storage/extsdcard/adas.mp4";

                    boolean is5880 = true;

                    String videoPath = is5880?path5880:nexus7Path;

                    FileInputStream e = new FileInputStream(new File(videoPath));
                    FileDescriptor fd = e.getFD();

                    mCreated = createStreamingMediaPlayer(fd);
                    doUVSwap(true);
                    if(mCreated) setPlayingStreamingMediaPlayer(true);
...
//                        try {
//                            FileInputStream e = new FileInputStream(new File(mSourceString));
//                            FileDescriptor fd = e.getFD();
//
//                            mCreated = createStreamingMediaPlayer(fd);
//
...
//        }
    ...
    public static native boolean createStreamingMediaPlayer(FileDescriptor fd);
    public static native void doUVSwap(boolean swap);
